...
public get(id: number){
  return this.databaseProvider.getDB().then((db: SQLiteObject) => {
    let sql = 'SELECT * FROM favorite WHERE product_id=?'; let params = [id];
  ...
  public getAllFavorite(name: string){
    return this.databaseProvider.getDB().then((db: SQLiteObject) => {
      ...
      let params: any[] = []; if(name){
        sql += ' WHERE p.name like ?'; params.push('%' + name + '%');
      }
      return db.executeSql(sql, params).then((data: any) =>
      {
        if(data.rows.length > 0){ let products: any[] = [];
          for (var i = 0; i < data.rows.length; i++) { var product = data.rows.item(i); products.push(product);
          }
          return products;
        }else{
          return [];
  ...
  public addFavorite(product: Product){
    return this.databaseProvider.getDB().then((db: SQLiteObject) => {
      ...
      let params =  [product.id]; return db.executeSql(sql, params)
      .then(() => console.log('Product berhasil ditambahkanke favorite!'))
  ...
  public removeFavorite(id: number){
    return this.databaseProvider.getDB().then((db: SQLiteObject) => {
      let sql = 'DELETE FROM favorite WHERE product_id=?'; let params = [id];
